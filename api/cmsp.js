  ...
  try {
    ...
    if (acao === 'login') {
      ...
      return res.status(200).json({
        ...
        codigoAluno: data.DadosUsuario?.CD_ALUNO || data.DadosUsuario?.CD_USUARIO,
        ra: data.statusRetorno || data.DadosUsuario?.NR_RA,
    ...
    // ── FALTAS ──
    if (acao === 'faltas') {
      const codigoAlunoFinal = codigoAluno || cdUsuario;
      console.log('Buscando faltas para codigoAluno:', codigoAlunoFinal);
      const r = await fetch(`${SED}/apiboletim/api/Frequencia/GetFaltasBimestreAtual?codigoAluno=${codigoAlunoFinal}`, {
        headers: authHeaders
      });
      const data = await safeJson(r);
