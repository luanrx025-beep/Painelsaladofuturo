...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === 'login') {
      ...
      // Extrai codigoAluno do statusRetorno (ex: "0001123674929SP" -> "000112367492")
      const raRaw = data.statusRetorno || '';
      const raNumeros = raRaw.replace(/[^0-9]/g, '');
      const codigoAluno = raNumeros.slice(0, -1);
      console.log('statusRetorno:', raRaw, '-> codigoAluno:', codigoAluno);
      console.log('DadosUsuario keys:', data.DadosUsuario ? Object.keys(data.DadosUsuario) : 'null');
    ...
    if (acao === 'faltas') {
      ...
      if (Array.isArray(data)) {
        console.log('Faltas: array com', data.length, 'itens. Primeiro item keys:', data[0] ? Object.keys(data[0]) : 'vazio');
        // Pega apenas os dados necessários para não retornar 236MB
        const resumo = Array.isArray(data) ? data.map(d => ({
          totalFaltasBimestre: d.totalFaltasBimestre ?? d.TotalFaltasBimestre ?? d.faltas ?? 0,
          totalAulasRealizadas: d.totalAulasRealizadas ?? d.TotalAulasRealizadas ?? d.aulas ?? 0,
          porcentagemFaltas: d.porcentagemFaltas ?? d.PorcentagemFaltas ?? 0,
          nivelPorcentagemFaltas: d.nivelPorcentagemFaltas ?? d.NivelPorcentagemFaltas ?? 0,
          disciplina: d.disciplina ?? d.Disciplina ?? d.nmDisciplina ?? '',
        })) : [];
        return res.status(200).json({ data: resumo });
      }
      
      console.log('Faltas keys:', Object.keys(data));
