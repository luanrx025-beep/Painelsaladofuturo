...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === 'login') {
      ...
      // Loga TODOS os campos disponíveis para debug
      console.log('DadosUsuario COMPLETO:', JSON.stringify(data.DadosUsuario));
      console.log('statusRetorno:', data.statusRetorno);

      // Tenta extrair codigoAluno de vários campos possíveis
    ...
    // ── FALTAS ──
    if (acao === 'faltas') {
      // Loga exatamente o que chegou
      console.log('FALTAS body recebido - codigoAluno:', codigoAluno, 'cdUsuario:', cdUsuario);

      // Tenta com codigoAluno, depois cdUsuario, depois sem último dígito
      const codigos = [
        codigoAluno,
        cdUsuario,
        codigoAluno ? codigoAluno.slice(0, -1) : null,
        cdUsuario ? String(cdUsuario).slice(0, -1) : null,
      ].filter(Boolean);

      console.log('Tentando códigos:', codigos);

      for (const cod of codigos) {
        const url = `${SED}/apiboletim/api/Frequencia/GetFaltasBimestreAtual?codigoAluno=${cod}`;
        console.log('Tentando URL:', url);
        const r = await fetch(url, { headers: authHeaders });
        const text = await r.text();
        console.log(`Código ${cod} -> status ${r.status} -> ${text.slice(0, 200)}`);

        let data;
        try { data = JSON.parse(text); } catch { continue; }
        if (!data) continue;

        // Verifica se tem dados reais (não vazio)
        const lista = Array.isArray(data) ? data :
                      Array.isArray(data.data) ? data.data : null;

        if (lista && lista.length > 0) {
          console.log('ACHOU dados com código:', cod, '- keys:', Object.keys(lista[0]));
          return res.status(200).json({ data: lista, codigoUsado: cod });
        }

        // Mesmo vazio, retorna se isSuccess
        if (data.isSuccess !== undefined) {
          console.log('Resposta isSuccess com código:', cod, '- data length:', (data.data||[]).length);
          // Continua tentando outros códigos se data estiver vazio
          if ((data.data || []).length > 0) {
            return res.status(200).json({ data: data.data, codigoUsado: cod });
          }
        }
      }

      // Nenhum código funcionou
      console.log('Nenhum código retornou dados');
      return res.status(200).json({ data: [], mensagem: 'Nenhuma falta encontrada com os códigos testados' });
    }
