...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === 'faltas') {
      ...
      // Tenta diferentes endpoints da SED
      const endpoints = [
        `${SED}/apiboletim/api/Frequencia/GetFaltasBimestreAtual?codigoAluno=${cod}`,
        `${SED}/apiboletim/api/Frequencia/GetFaltasBimestre?codigoAluno=${cod}&anoLetivo=${ano}`,
        `${SED}/apiboletim/api/Frequencia/GetFrequencia?codigoAluno=${cod}&anoLetivo=${ano}`,
        `${SED}/apiboletim/api/Frequencia/GetFaltasAluno?codigoAluno=${cod}`,
        `${SED}/apiboletim/api/Boletim/GetBoletim?codigoAluno=${cod}&anoLetivo=${ano}`,
      ];
      ...
      for (const url of endpoints) {
        try {
          ...
          const text = await r.text();
          console.log(`URL: ${url.split('api/')[1]} -> ${r.status} -> ${text.slice(0,150)}`);
          if (!r.ok) continue;
          ...
          if (lista && lista.length > 0) {
            console.log('ACHOU! Keys:', Object.keys(lista[0]));
            return res.status(200).json({ data: lista });
          }
        } catch(e) { console.log('Erro endpoint:', e.message); }
      }

      return res.status(200).json({ data: [], mensagem: 'Sem faltas registradas no bimestre atual' });
    }
