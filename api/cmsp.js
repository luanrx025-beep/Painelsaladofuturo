...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === 'faltas') {
      ...
      const text = await r.text();
      console.log('Faltas status:', r.status);
      console.log('Faltas raw (primeiros 500 chars):', text.slice(0, 500));
      
      let data;
      try { data = JSON.parse(text); } catch { return res.status(502).json({ error: 'Resposta inválida da SED' }); }
      if (!data) return res.status(502).json({ error: 'Serviço de faltas indisponível.' });

      // Normaliza qualquer formato
      const lista = Array.isArray(data) ? data : (data.data ? (Array.isArray(data.data) ? data.data : [data.data]) : [data]);
      console.log('Faltas lista length:', lista.length);
      if (lista[0]) console.log('Faltas primeiro item keys:', Object.keys(lista[0]));
      
      const resumo = lista.map(d => ({
        totalFaltasBimestre: d.totalFaltasBimestre ?? d.TotalFaltasBimestre ?? d.Faltas ?? d.faltas ?? 0,
        totalAulasRealizadas: d.totalAulasRealizadas ?? d.TotalAulasRealizadas ?? d.Aulas ?? d.aulas ?? 0,
        porcentagemFaltas: d.porcentagemFaltas ?? d.PorcentagemFaltas ?? d.Percentual ?? 0,
        nivelPorcentagemFaltas: d.nivelPorcentagemFaltas ?? d.NivelPorcentagemFaltas ?? 0,
        disciplina: d.disciplina ?? d.Disciplina ?? d.nmDisciplina ?? d.NmDisciplina ?? '',
      }));
