  ...
  try {
    ...
    if (acao === 'login') {
      ...
      try {
        ...
        console.log('Turma:', r2.status, t2.slice(0,200));
        const d2 = JSON.parse(t2);
        const td = Array.isArray(d2?.data) ? d2.data[0] : (d2?.data || d2 || {});
        turma   = td?.DS_TURMA || td?.NM_TURMA || td?.TURMA || td?.descricao || '';
        escola  = td?.NM_ESCOLA || td?.DS_ESCOLA || td?.ESCOLA || '';
        professor = td?.NM_PROFESSOR || td?.PROFESSOR || '';
        console.log('turma:', turma, 'escola:', escola);
      } catch(e) { console.log('Err turma:', e.message); }
